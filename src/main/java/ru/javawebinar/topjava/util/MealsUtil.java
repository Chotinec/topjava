...
public class MealsUtil {
    ...
    static {
        Meal meal = new Meal(IdCounter.incrementAndGetCounter(), LocalDateTime.of(2015, Month.MAY, 30, 10, 0), "Завтрак", 500);
        meals.put(meal.getId(), meal);
        meal = new Meal(IdCounter.incrementAndGetCounter(), LocalDateTime.of(2015, Month.MAY, 30, 13, 0), "Обед", 1000);
        meals.put(meal.getId(), meal);
        meal = new Meal(IdCounter.incrementAndGetCounter(), LocalDateTime.of(2015, Month.MAY, 30, 20, 0), "Ужин", 500);
        meals.put(meal.getId(), meal);
        meal = new Meal(IdCounter.incrementAndGetCounter(), LocalDateTime.of(2015, Month.MAY, 31, 10, 0), "Завтрак", 1000);
        meals.put(meal.getId(), meal);
        meal = new Meal(IdCounter.incrementAndGetCounter(), LocalDateTime.of(2015, Month.MAY, 31, 13, 0), "Обед", 500);
        meals.put(meal.getId(), meal);
        meal = new Meal(IdCounter.incrementAndGetCounter(), LocalDateTime.of(2015, Month.MAY, 31, 20, 0), "Ужин", 510);
        meals.put(meal.getId(), meal);
    }
    ...
    public static void main(String[] args) {
        List<MealWithExceed> mealsWithExceeded = getFilteredWithExceeded(new ArrayList<>(meals.values()), LocalTime.of(7, 0), LocalTime.of(12, 0), CALORIES_PERDAY);
...
//                      Collectors.toMap(Meal::getDate, Meal::getCalories, Integer::sum)
    ...
    public static List<MealWithExceed> convertWithExceeded(List<Meal> meals, int caloriesPerDay) {
        Map<LocalDate, Integer> caloriesSumByDate = meals.stream()
                .collect(
                        Collectors.groupingBy(Meal::getDate, Collectors.summingInt(Meal::getCalories))
//                      Collectors.toMap(Meal::getDate, Meal::getCalories, Integer::sum)
